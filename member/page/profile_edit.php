...
<body>
    <?php
        if(isset($_POST['edit'])){
            ...
            $tel = $_POST['tel'];
            $sql2 = "UPDATE member set name_mem = '$name_mem', tel = '$tel' WHERE id_mem = '$id_mem'";
            mysqli_query($conn,$sql2);
            header('Refresh:0; url=./dashboard.php?page=profile_edit');
        }
    ?>
    <form action="./dashboard.php?page=profile_edit" method="POST">
        <table class="table table-bordered table-responsive bg-white rounded shadow-sm ">
            <thead>
                <tr>
                    <th scope="col" colspan="2" class="fs-3" ><center>ฟอร์มแก้ไขข้อมูล</center></th>
                </tr>
            </thead>
            <tbody>
                <tr>
                ...
                <tr>
                    ...
                    <td><input type="text" class="form-control" name="name_mem" value="<?php echo $row['name_mem']; ?>" ></td>
                </tr>
                <tr>
                    <th scope="row" class="align-middle"><center>เบอร์โทรศัพท์</center></th>
                    <td><input type="text" class="form-control" name="tel" value="<?php echo $row['tel']; ?>" ></td>
                </tr>
                <tr>
                    <th colspan="2">
                        <center><input type="submit" class="btn btn-primary w-50" name="edit" value="บันทึก"></center>
                    </th>
                </tr>
            </tbody>
        </table>
